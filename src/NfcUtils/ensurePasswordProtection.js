...
async function ensurePasswordProtection() {
  ...
  if (auth === 255) {
    ...
    writeRespBytes = await NfcManager.nfcAHandler.transceive([0xa2, authPageIdx + 2, ...password]);
    console.warn(writeRespBytes);
    ...
    writeRespBytes = await NfcManager.nfcAHandler.transceive([0x1b, ...password]);
    console.warn(writeRespBytes);
